...
source /dev/stdin <<< "$(curl -fsSL https://raw.githubusercontent.com/Igor-Pchelko/dotfiles/mac/script/prompt)"
...
echo ''
read -p "Do you want to use Igor-Pchelko's dotfiles? y/n" -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    echo ''
	echo "Now pulling down Igor-Pchelko dotfiles..."
	git clone https://github.com/Igor-Pchelko/dotfiles.git ~/.dotfiles
	echo ''
    ...
    if [[ $? -eq 0 ]]
    then
        echo "Successfully configured your environment with Igor-Pchelko's macOS dotfiles..."
    else
        echo "Igor-Pchelko's macOS dotfiles were not applied successfully..." >&2
fi
else 
	echo ''
    echo "You chose not to apply Igor-Pchelko's macOS dotfiles. You will need to configure your environment manually..."
	echo ''
